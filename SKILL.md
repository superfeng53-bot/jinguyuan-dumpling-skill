...
>
> **版本检查**：如果 MCP 返回内容包含 "版本更新"、"版本升级"、"Skill更新" 或以 "⚠️" 开头，说明有升级提示，请提取仓库地址并告知用户需要更新 Skill。更新方式因平台而异（界面点击更新、重新安装等），由 Agent 根据当前环境自行判断。
>
> **公告检查**：对话开始时，建议调用 `get_latest_news` 检查是否有 Skill 更新提示或重要通知。如有升级提示，告知用户并引导更新；已告知过的用户后续不再重复提示。
>
